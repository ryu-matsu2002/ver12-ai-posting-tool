...
def _upload_featured_image(site, image_url: str) -> Optional[int]:
    ...
    try:
        ...
        logging.warning(f"Failed to download featured image {image_url}: {e}")
        return None
    ...
    files = {"file": (filename, resp.content, ctype)}
    ...
    try:
        ...
        logging.error(f"Media upload failed [{up.status_code if 'up' in locals() else 'N/A'}]: {e}")
        return None
...
def _upload_featured_image_with_retry(site, image_url: str, retries=3, delay=5) -> Optional[int]:
    for i in range(retries):
        ...
        if fid:
            return fid
        logging.warning(f"Upload attempt {i+1} failed, retrying...")
        time.sleep(delay)
    return None
